...
class CareLib_Breadcrumbs {
	...
	public function trail() {
		...
		if ( ! empty( $this->items ) && is_array( $this->items ) ) {
			...
			$breadcrumb = "\n\t\t" . '<' . tag_escape( $this->args['container'] ) . ' role="navigation" aria-label="' . esc_attr_x( 'Breadcrumbs', 'Breadcrumbs aria label', 'carelib' ) . '" class="breadcrumb-trail breadcrumbs" itemprop="breadcrumb">';
	...
	public function default_labels() {
		...
		$labels = array(
			'browse'              => esc_html__( 'Browse:',                             'carelib' ),
			'home'                => esc_html__( 'Home',                                'carelib' ),
			'error_404'           => esc_html__( '404 Not Found',                       'carelib' ),
			'archives'            => esc_html__( 'Archives',                            'carelib' ),
			/* Translators: %s is the search query. The HTML entities are opening and closing curly quotes. */
			'search'              => esc_html__( 'Search results for &#8220;%s&#8221;', 'carelib' ),
			/* Translators: %s is the page number. */
			'paged'               => esc_html__( 'Page %s',                             'carelib' ),
			/* Translators: Minute archive title. %s is the minute time format. */
			'archive_minute'      => esc_html__( 'Minute %s',                           'carelib' ),
			/* Translators: Weekly archive title. %s is the week date format. */
			'archive_week'        => esc_html__( 'Week %s',                             'carelib' ),
	...
	public function do_minute_hour_archive_items() {
		...
		if ( true === $this->args['show_title'] )
			$this->items[] = sprintf( $this->args['labels']['archive_minute_hour'], get_the_time( esc_html_x( 'g:i a', 'minute and hour archives time format', 'carelib' ) ) );
	}
	...
	public function do_minute_archive_items() {
		...
		/* Add the minute item. */
		if ( true === $this->args['show_title'] )
			$this->items[] = sprintf( $this->args['labels']['archive_minute'], get_the_time( esc_html_x( 'i', 'minute archives time format', 'carelib' ) ) );
	}
	...
	public function do_hour_archive_items() {
		...
		/* Add the hour item. */
		if ( true === $this->args['show_title'] )
			$this->items[] = sprintf( $this->args['labels']['archive_hour'], get_the_time( esc_html_x( 'g a', 'hour archives time format', 'carelib' ) ) );
	}
	...
	public function do_day_archive_items() {
		...
		$year  = sprintf( $this->args['labels']['archive_year'],  get_the_time( esc_html_x( 'Y', 'yearly archives date format',  'carelib' ) ) );
		$month = sprintf( $this->args['labels']['archive_month'], get_the_time( esc_html_x( 'F', 'monthly archives date format', 'carelib' ) ) );
		$day   = sprintf( $this->args['labels']['archive_day'],   get_the_time( esc_html_x( 'j', 'daily archives date format',   'carelib' ) ) );
	...
	public function do_week_archive_items() {
		...
		$year = sprintf( $this->args['labels']['archive_year'],  get_the_time( esc_html_x( 'Y', 'yearly archives date format', 'carelib' ) ) );
		$week = sprintf( $this->args['labels']['archive_week'],  get_the_time( esc_html_x( 'W', 'weekly archives date format', 'carelib' ) ) );
	...
	public function map_rewrite_tags( $post_id, $path ) {
		...
		if ( is_array( $matches ) ) {
			...
			foreach ( $matches as $match ) {
				...
				if ( '%year%' == $tag )
					$this->items[] = '<a href="' . esc_url( get_year_link( get_the_time( 'Y', $post_id ) ) ) . '">' . sprintf( $this->args['labels']['archive_year'], get_the_time( esc_html_x( 'Y', 'yearly archives date format',  'carelib' ) ) ) . '</a>';

				/* If using the %monthnum% tag, add a link to the monthly archive. */
				elseif ( '%monthnum%' == $tag )
					$this->items[] = '<a href="' . esc_url( get_month_link( get_the_time( 'Y', $post_id ), get_the_time( 'm', $post_id ) ) ) . '">' . sprintf( $this->args['labels']['archive_month'], get_the_time( esc_html_x( 'F', 'monthly archives date format', 'carelib' ) ) ) . '</a>';

				/* If using the %day% tag, add a link to the daily archive. */
				elseif ( '%day%' == $tag )
					$this->items[] = '<a href="' . esc_url( get_day_link( get_the_time( 'Y', $post_id ), get_the_time( 'm', $post_id ), get_the_time( 'd', $post_id ) ) ) . '">' . sprintf( $this->args['labels']['archive_day'], get_the_time( esc_html_x( 'j', 'daily archives date format', 'carelib' ) ) ) . '</a>';
