...
add_action( 'add_meta_boxes', 'carelib_add_post_style_meta_box', 10, 2 );

# Saves the post meta box data.
add_action( 'save_post',       'carelib_save_post_style', 10, 2 );
add_action( 'add_attachment',  'carelib_save_post_style'        );
add_action( 'edit_attachment', 'carelib_save_post_style'        );
 ...
 * @return void
 */
function carelib_add_post_style_meta_box( $post_type, $post ) {

	$styles = carelib_get_post_styles( $post_type );

	if ( ! empty( $styles ) && current_user_can( 'edit_theme_options' ) )
		add_meta_box( 'hybrid-post-style', esc_html__( 'Style', 'carelib' ), 'carelib_post_style_meta_box', $post_type, 'side', 'default' );
}
 ...
 * @return void
 */
function carelib_post_style_meta_box( $post, $box ) {
 ...
 * @return void|int
 */
function carelib_save_post_style( $post_id, $post = '' ) {
	...
	$meta_value = carelib_get_post_style( $post_id );
	...
	if ( '' == $new_meta_value && $meta_value )
		carelib_delete_post_style( $post_id );

	// If a new meta value was added and there was no previous value, add it.
	elseif ( $meta_value !== $new_meta_value )
		carelib_set_post_style( $post_id, $new_meta_value );
}
