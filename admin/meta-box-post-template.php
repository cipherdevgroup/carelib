...
add_action( 'add_meta_boxes', 'carelib_meta_box_post_add_template',    10, 2 );

# Save the post template meta box data on the 'save_post' hook.
add_action( 'save_post',       'carelib_meta_box_post_save_template', 10, 2 );
add_action( 'add_attachment',  'carelib_meta_box_post_save_template'        );
add_action( 'edit_attachment', 'carelib_meta_box_post_save_template'        );
 ...
 * @return void
 */
function carelib_meta_box_post_add_template( $post_type, $post ) {
 ...
 * @return void
 */
function carelib_meta_box_post_display_template( $post, $box ) {
 ...
 * @return void|int
 */
function carelib_meta_box_post_save_template( $post_id, $post = '' ) {
	...
	$meta_value = carelib_get_post_template( $post_id );

	// If there is no new meta value but an old value exists, delete it.
	if ( '' == $new_meta_value && $meta_value )
		carelib_delete_post_template( $post_id );

	// If the new meta value does not match the old value, update it.
	elseif ( $new_meta_value != $meta_value )
		carelib_set_post_template( $post_id, $new_meta_value );
}
