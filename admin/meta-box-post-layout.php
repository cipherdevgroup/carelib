...
add_action( 'add_meta_boxes', 'carelib_add_post_layout_meta_box', 10, 2 );

# Saves the post layout on the post editing page.
add_action( 'save_post',       'carelib_save_post_layout', 10, 2 );
add_action( 'add_attachment',  'carelib_save_post_layout'        );
add_action( 'edit_attachment', 'carelib_save_post_layout'        );
 ...
 * @return void
 */
function carelib_add_post_layout_meta_box( $post_type ) {
	...
	if ( current_theme_supports( 'theme-layouts', 'post_meta' ) && post_type_supports( $post_type, 'theme-layouts' ) && current_user_can( 'edit_theme_options' ) ) {
		...
		// Add meta box.
		add_meta_box( 'hybrid-post-layout', esc_html__( 'Layout', 'carelib' ), 'carelib_post_layout_meta_box', $post_type, 'side', 'default' );

		// Load scripts/styles.
		add_action( 'admin_enqueue_scripts', 'carelib_post_layout_enqueue', 5 );
	}
}
 ...
 * @return void
 */
function carelib_post_layout_enqueue() {
 ...
 * @return void
 */
function carelib_post_layout_meta_box( $post, $box ) {
 ...
 * @return void|int
 */
function carelib_save_post_layout( $post_id, $post = '' ) {
	...
	$meta_value = carelib_get_post_layout( $post_id );
	...
	if ( '' == $new_meta_value && $meta_value )
		carelib_delete_post_layout( $post_id );

	// If a new meta value was added and there was no previous value, add it.
	elseif ( $meta_value !== $new_meta_value )
		carelib_set_post_layout( $post_id, $new_meta_value );
}
